package com.mage.platform.framework.freemarker;
...
import com.mage.platform.utils.FreeMarkerUtil;
import com.mage.platform.utils.HtmlUtil;
...
public final class FreeMarkerPaser implements Cloneable{
	...
	public String proessPageContent(){
		try {
			...
			//重新设置引入的资源文件路径
			if(wrapPath){
				content = HtmlUtil.wrapjavascript(content, this.getResPath());
				content =  HtmlUtil.wrapcss(content, getResPath());
			}
			return content;
		} catch (IOException e) {
			e.printStackTrace();
