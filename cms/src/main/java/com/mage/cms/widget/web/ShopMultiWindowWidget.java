 ...
 */
@UrlPathService(pluginType="3",name="多标签橱窗插件")
@Service
public class ShopMultiWindowWidget extends AbstractCmsWidget {
	protected void display(Map<String, String> params) {
		//this.putData("data", this.storeLay.getData());//存储数据
		this.freeMarkerPaser.setPageName("ShopMultiWindowWidgetStatic");
	}
	...
	@UrlPathMethod(name = "添加", path = "/Store/AddStoreLayout3")
	public CmsLineResp add(String json) throws Exception {
		...
		return cmsReq.add(new CmsLayOutContentPraser() {
			...
			public String genModualCnt(Modual modual) {
				...
				showcase.setType(CmsConst.WINDOW_MULTI_TYPE_1);
				showcase.setStyle(0);
	...
	@UrlPathMethod(name = "删除", path = "/Store/DeleteStoreLayout3")
	...
	public StaticLayOutExtVo getStcStoreLayOutParamter(StoreLayOutResp storeLayOutResp)throws Exception {
		...
		vo.setLabels(cmsResp.getLabels());
		return vo;
	}
