	...
	b.fn.fullpage = function(c) {
		...
		function V() {
			...
			if ((!c.autoScrolling || c.scrollBar) && !a.hasClass("active")) {
				...
				if (J.length) var m = J.data("anchor"),
					k = J.index();
			...
			c.scrollBar && (clearTimeout(X), X = setTimeout(function() {
		...
		function ua(a) {
			...
			if (!Z(a.target)) {
				...
				var e = Y(a);
				p || w || (d = aa(d), u = d.y, C = d.x, a.find(".fp-slides").length && Math.abs(D - C) > Math.abs(x - u) ? Math.abs(D - C) > b(window).width() / 100 * c.touchSensitivity && (D > C ? m.right && b.fn.fullpage.moveSlideRight() : m.left && b.fn.fullpage.moveSlideLeft()) : c.autoScrolling && Math.abs(x - u) > b(window).height() / 100 * c.touchSensitivity && (x > u ? B("down", e) : u > x && B("up", e)))
			}
		}
		function Z(a, d) {
			d = d || 0;
		...
		function r(a, d, e) {
			...
			if ("undefined" !== typeof f && (d = {
				...
				c.autoScrolling && c.continuousVertical && "undefined" !== typeof d.isMovementUp && (!d.isMovementUp && "up" == d.yMovement || d.isMovementUp && "down" == d.yMovement) && (d.isMovementUp ? b(".fp-section.active").before(d.activeSection.nextAll(".fp-section")) : b(".fp-section.active").after(d.activeSection.prevAll(".fp-section").get().reverse()), k(b(".fp-section.active").position().top), ca(), d.wrapAroundElements = d.activeSection, d.dest = d.element.position(), d.dtop = d.dest.top, d.yMovement = I(d.element));
				a.addClass("active").siblings().removeClass("active");
				p = !0;
		...
		function wa(a) {
			if (c.css3 && c.autoScrolling && !c.scrollBar) da("translate3d(0px, -" + a.dtop + "px, 0px)", !0), setTimeout(function() {
				ea(a)
			}, c.scrollingSpeed);
			else {
				var d = xa(a);
		...
		function y(a, d) {
			...
			c.css3 ? (e = "translate3d(-" + e.left + "px, 0px, 0px)", ha(a.find(".fp-slidesContainer"), 0 < c.scrollingSpeed).css(ia(e)), setTimeout(function() {
				u()
		...
		function ka() {
			if (c.responsive) {
				...
				b(window).width() < c.responsive ? a || (b.fn.fullpage.setAutoScrolling(!1, "internal"), b("#fp-nav").hide(), g.addClass("fp-responsive")) : a && (b.fn.fullpage.setAutoScrolling(Q.autoScrolling, "internal"), b("#fp-nav").show(), g.removeClass("fp-responsive"))
			}
		}
		function ha(a) {
		...
		function O(a, d) {
			...
			a === q || c.hasClass("active") ? pa(c, d) : r(c, function() {
				pa(c, d)
			})
		}
		function pa(a, b) {
		...
		function L(a, b, e, f) {
			var g = "";
			c.anchors.length ? (a ? ("undefined" !== typeof e && (g = e), "undefined" === typeof b && (b = a), N = b, qa(g + "/" + b)) : ("undefined" !== typeof a && (N = b), qa(e)), F(location.hash)) : "undefined" !== typeof a ? F(f + "-" + a) : F(String(f))
		}
		function qa(a) {
		...
		function Ca() {
			if (E || S) {
				var a = ra();
				b(document).off("touchstart " + a.down).on("touchstart " + a.down, va);
				b(document).off("touchmove " + a.move).on("touchmove " + a.move, ua)
			}
		}
		function Da() {
			if (E || S) {
		...
		b.fn.fullpage.setAutoScrolling = function(a, d) {
			...
			c.autoScrolling && !c.scrollBar ? (b("html, body").css({
				...
				"touch-action": "none"
			}), e.length && k(e.position().top)) : (b("html, body").css({
				overflow: "visible",
				height: "initial"
			}), b.fn.fullpage.setRecordHistory(!1, "internal"), g.css({
				"-ms-touch-action": "",
				"touch-action": ""
			}), k(0), b("html, body").scrollTop(e.position().top))
		};
		...
		b.fn.fullpage.setMouseWheelScrolling = function(a) {
			a ? document.addEventListener ? (document.addEventListener("mousewheel", t, !1), document.addEventListener("wheel", t, !1)) : document.attachEvent("onmousewheel", t) : document.addEventListener ? (document.removeEventListener("mousewheel", t, !1), document.removeEventListener("wheel", t, !1)) : document.detachEvent("onmousewheel", t)
		};
		b.fn.fullpage.setAllowScrolling = function(a, c) {
			"undefined" != typeof c ? (c = c.replace(" ", "").split(","), b.each(c, function(c, d) {
				switch (d) {
				case "up":
					m.up = a;
					break;
			...
			})) : a ? (b.fn.fullpage.setMouseWheelScrolling(!0), Ca()) : (b.fn.fullpage.setMouseWheelScrolling(!1), Da())
		};
		...
		b.fn.fullpage.reBuild = function(a) {
			...
			b(".fp-section").each(function() {
				...
				c.verticalCentered && b(this).find(".fp-tableCell").css("height", oa(b(this)) + "px");
				b(this).css("height", l + "px");
				c.scrollOverflow && (d.length ? d.each(function() {
					z(b(this))
				}) : z(b(this)));
		...
		b(this).length ? (g.css({
			height: "100%",
			position: "relative"
		}), g.addClass(fa)) : G("error", "Error! Fullpage.js needs to be initialized with a selector. For example: $('#myContainer').fullpage();");
		...
		b(".fp-section").each(function(a) {
			...
			if (a.length) {
				...
				!c.animateAnchor && d.length && (c.autoScrolling ? k(d.position().top) : (k(0), F(a), b("html, body").scrollTop(d.position().top)), K(a, null), b.isFunction(c.afterLoad) && c.afterLoad.call(d, a, d.index(".fp-section") + 1), d.addClass("active").siblings().removeClass("active"))
			}
		...
		b(document).keydown(function(a) {
			if (c.keyboardScrolling && c.autoScrolling && (40 != a.which && 38 != a.which || a.preventDefault(), !p)) switch (a.which) {
			case 38:
			case 33:
